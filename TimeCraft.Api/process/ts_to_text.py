...
def optimize_text_with_llm(text, deployment_name=AZURE_OPENAI_DEPLOYMENT_NAME, temperature=0.3, max_tokens=300):
    """
    Optimize a text description using Azure OpenAI to improve grammar, clarity, and narrative quality.
    
    Args:
        text (str): The raw description to be optimized.
        deployment_name (str): Azure OpenAI deployment name.
        temperature (float): Sampling temperature.
        max_tokens (int): Max token limit for response.
    
    Returns:
        str: Optimized text.
    """
    if not all([AZURE_OPENAI_API_KEY, AZURE_OPENAI_ENDPOINT, deployment_name]):
        print("Azure OpenAI environment variables not set correctly.")
        return text
...
def generate_text_description_for_time_series(file_path, prediction_length, dataset_name,
    ...
    for column in df.columns:
        ...
        for start in tqdm(range(0, len(df), 1), desc=f"Processing column {column}"):
            ...
            # Store both descriptions in DataFrame
