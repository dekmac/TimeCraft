...
class DatasetDC(Dataset):
    def __init__(self, seq: torch.Tensor, cls: torch.Tensor) -> None:
        super().__init__()
        assert seq.shape[0] == cls.shape[0], "Sequence and class tensor must have the same length."
        self.seq = seq.clone()
        self.cls = cls.clone()
    ...
    def __getitem__(self, index: int) -> tuple:
        return self.seq[index], self.cls[index]


class DataModuleDC(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        self.vali_bin_index = np.digitize(vali_metrics, bins_edge) - 1
        self.transform()
    ...
    def train_dataloader(self):
        train_dataset = DatasetDC(torch.from_numpy(self.train_data), torch.from_numpy(self.bin_index))
    ...
    def val_dataloader(self):
        vali_dataset = DatasetDC(torch.from_numpy(self.vali_data), torch.from_numpy(self.vali_bin_index))
...
class UnetDC(nn.Module):
    ...
    def forward(self, x, time, classes, cond_drop_prob = None, x_self_cond = None):
        ...
        classes_emb = self.classes_emb(classes).squeeze(1)
