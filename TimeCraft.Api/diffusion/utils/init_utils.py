...
from diffusion.utils.cli_utils import nondefault_trainer_args
from diffusion.utils.callback_utils import prepare_trainer_configs
...
def init_model_data_trainer(parser):
    ...
    if opt.use_text:
        print(f"Text-conditioned generation enabled.")
        nowname += "_text"
        assert config.data.params.input_channels == 1, \
            "Text-conditioned generation (use_text=True) requires univariate time series (input_channels == 1)."
