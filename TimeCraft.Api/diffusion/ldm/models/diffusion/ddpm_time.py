...
from ldm.models.diffusion.guided_ddim_time import  GuideDDIMSampler
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        label = None if 'label' not in batch else batch['label'].long()
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
    ...
    def shared_step(self, batch, **kwargs):
        tp = self.get_input(batch, self.first_stage_key)
        if len(tp) == 3:
            x, c, label = tp
        else:
            x, c= tp
    ...
    def forward(self, x, c, label=None, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
    ...
    def apply_model(self, x_noisy, t, cond, mask, label=None, cfg_scale=1, cond_drop_prob=None,
    ...
    def p_losses(self, x_start, condmask, t, label=None, noise=None, data_key=None, text_embedding=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    @torch.no_grad()
    def sample_log(self, cond,batch_size,ddim, ddim_steps=20, label=None, IFCalculater=None, **kwargs):

        if ddim:
            if IFCalculater is not None:
                guided_sampler = GuideDDIMSampler(self,
                                    guide_type='IFC',
                                    GDCalculater=IFCalculater)
    ...
    def log_images(self, batch, N=8, n_row=8, sample=True, plot_reconstruction=False, 
        ...
        if len(tp) == 6:
            z, c, x, xrec, xc, mask = tp 
        else:
            z, c, x, xrec, xc, mask, label = tp
        N = min(x.shape[0], N)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, label=None, c_crossattn: list = None, cond_drop_prob=0., mask=None, text_embedding=None, **kwargs):
    ...
    def cfg_forward(self, x, t, label=None, c_crossattn: list = None, mask=None, text_embedding=None, **kwargs):
        if (c_crossattn is not None) and (not None in c_crossattn):
            cc = torch.cat(c_crossattn, 1)
        else:
            cc = None
