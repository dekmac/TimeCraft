...
# === Azure-Compatible Model Selection ===
AZURE_ENGINE = os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME", "gpt-4o")

@retry_parse_fail_prone_cmd
def autofb_commongen(concepts: List[str], max_attempts: int) -> str:
    task_init = CommongenTaskInit(engine=AZURE_ENGINE, prompt_examples="data/prompt/commongen/init.jsonl")
    task_feedback = CommongenFeedback(engine=AZURE_ENGINE, prompt_examples="data/prompt/commongen/feedback.jsonl")
    task_iterate = CommongenTaskIterate(engine=AZURE_ENGINE, prompt_examples="data/prompt/commongen/iterate.jsonl")
...
def run_cmd():
    concepts = sys.argv[2:]
    max_attempts = 5
    sent_to_fb = autofb_commongen(concepts=concepts, max_attempts=max_attempts)
...
def run_multi_sample(inputs_file_path: str, n_samples: int = 4):
    ...
    task_init = CommongenTaskInit(engine=AZURE_ENGINE, prompt_examples="data/prompt/commongen/init.jsonl")
    task_feedback = CommongenFeedback(engine=AZURE_ENGINE, prompt_examples="data/prompt/commongen/feedback.v1.jsonl")
    ...
    output_path = inputs_file_path + "." + AZURE_ENGINE + (".multi.out" if not is_rerun else ".v0")
    version = 0
    while pathlib.Path(output_path).exists():
        output_path = output_path + f".v{version}"
        version += 1
